...
class HealthManager:
    ...
    def call_health_script(self, status: str, additional_info: str) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                logger.debug(f"Health script executed successfully: {result.stdout.strip()}")
                return True
            else:
                logger.warning(f"Health script failed with return code {result.returncode}: {result.stderr.strip()}")
                return False
    ...
    def report_render_success(self, additional_info: str = "Display render completed successfully"):
    ...
    def recheck_health(self) -> bool:
        ...
        logger.info(f"Re-checking health status ({interval_minutes:.0f}+ minutes since last call)")
