...
import requests
from PIL import Image, ImageColor, ImageDraw, ImageEnhance, ImageFont  # aka pillow
...
class Viewer(ABC):
    ...
    def process_image_position(self, img):
        ...
        if (
            (not img_width == self.image_width)
            or (not img_height == self.screen_height)
            or not self.scale_x == self.scale_y
        ):
...
class EinkViewer(Viewer):
    ...
    def update(self, image_path, img, title):
        ...
        self.update_thread = threading.Thread(
            target=self.display_image, args=(img, title)
        )
...
class RoonAlbumArt:
    def __init__(self, config, viewer):
        ...
        self.token_file = (
            ...
            / f".roon_album_display_token_test_{os.getpid()}.txt"
        )
    ...
    def save_server_to_config(self, server_ip, server_port):
        ...
        try:
            ...
            logger.info(
                f"Saved server details ({server_ip}:{server_port}) to config for future use"
            )
        except Exception as e:
    ...
    def connect_to_roon(self):
        ...
        try:
            if (
                "SERVER" in self.config
                and self.config.get("SERVER", "ip")
                and self.config.get("SERVER", "port")
            ):
                ...
                if server_ip and server_port:
                    logger.info(
                        f"Trying direct connection to saved server at {server_ip}:{server_port}"
                    )
                    try:
                        ...
                        if api and api.host:
                            ...
                            try:
                                ...
                                if zones is not None and zones:
                                    logger.info(
                                        "Successfully connected to saved server!"
                                    )
                            ...
                            logger.warning(
                                "API instance created but appears to be invalid"
                            )
    ...
    def discover_and_connect(self):
        ...
        logger.info(
            f"Connecting to first Roon server found at {server_ip}:{server_port}"
        )
        ...
        try:
            ...
            if token is None and api is not None:
                ...
                logger.info(
                    "Authorization successful, token saved for future connections"
                )
            ...
            logger.exception(
                f"Error connecting to Roon server at {server_ip}:{server_port}: {e}"
            )
            return None
                ...
                for i, zone_item in enumerate(data):
                    ...
                    if isinstance(zone_item, str):
                        ...
                        if zone_data:
                            ...
                            logger.warning(
                                f"No zone data found for zone ID: {zone_item}"
                            )

            else:
                logger.warning(
                    f"Unexpected data format in {event_type} event: {type(data)}"
                )
    ...
    def process_zone_data(self, zone_id, zone_data):
        ...
        try:
            ...
            logger.debug(
                f"Processing zone {zone_id}, data keys: {zone_data.keys() if isinstance(zone_data, dict) else 'not a dict'}"
            )

            name = zone_data["display_name"]
            if self.forbidden_zone_names and name in self.forbidden_zone_names:
                logger.debug(
                    f"Received event from zone {name} but it is in the forbidden list"
                )
                return False
            if self.allowed_zone_names and not name in self.allowed_zone_names:
                logger.debug(
                    f"Received event from zone {name} but it is not in the allowed list"
                )
                return False
            ...
            if isinstance(zone_data, dict):
                ...
                # Check if it might be in a nested structure
                    ...
                    if (
                        "now_playing" in zone_data["state"]
                ...
                # Check if this is a different API structure
                    ...
                    "display_name" in zone_data
                    and "queue" in zone_data
                    and "now_playing" in zone_data.get("queue", {})
                ):
    ...
    def process_now_playing(self, now_playing):
        ...
        try:
            logger.debug(
                f"Now playing keys: {now_playing.keys() if isinstance(now_playing, dict) else 'not a dict'}"
            )
            ...
            if image_key != self.last_image_key:
                ...
                if isinstance(now_playing, dict):
                    ...
                    if "three_line" in now_playing and isinstance(
                        now_playing["three_line"], dict
                    ):
                        track_info = now_playing["three_line"].get("line1", track_info)
                        artist_info = now_playing["three_line"].get(
                            "line2", artist_info
                        )
                        album_info = now_playing["three_line"].get("line3", album_info)
                    elif "two_line" in now_playing and isinstance(
                        now_playing["two_line"], dict
                    ):
                        track_info = now_playing["two_line"].get("line1", track_info)
                        artist_info = now_playing["two_line"].get("line2", artist_info)
                    elif "one_line" in now_playing and isinstance(
                        now_playing["one_line"], dict
                    ):
                ...
                logger.debug(
                    f"Update contains the same image as is currently displayed"
                )

        except Exception as e:
            logger.error(f"Error processing now_playing data: {e}")
            logger.debug(
                f"Now playing data that caused error: {str(now_playing)[:200]}..."
            )
        ...
        try:
            ...
            if os.path.exists(image_path):
                ...
                image_url = self.roon.get_image(
                    image_key, "fit", self.viewer.image_size, self.viewer.image_size
                )
                ...
                if not (
                    self.viewer.contrast_adjustment == 1
                    and self.viewer.colour_balance_adjustment == 1
                    and self.viewer.brightness_adjustment == 1
                    and self.viewer.sharpness_adjustment == 1
                ):
...
class RoonFrameConfig:
    ...
    def load_config(self):
        ...
        if not config_file.exists():
            logger.info(
                f"Configuration file {config_file} not found. Creating default config."
            )
...
def rnum(img):
    ...
    """Replace colors in an image according to the mapping"""
    ...
    # Convert color lists to RGB
    colors_rgb = [hex_to_rgb(color) for color in colors_real]
    colors_map_rgb = [hex_to_rgb(color) for color in colors_map]

    # Create a new array with the same shape
    result = pixels.copy()

    # For each color in the mapping
    for i, (original_rgb, target_rgb) in enumerate(zip(colors_rgb, colors_map_rgb)):
        # Create masks for each channel
        r_mask = pixels[:, :, 0] == original_rgb[0]
        g_mask = pixels[:, :, 1] == original_rgb[1]
        b_mask = pixels[:, :, 2] == original_rgb[2]

        # Combine masks to find pixels with exact RGB match
        color_mask = r_mask & g_mask & b_mask

        # Apply new color to matching pixels
        result[color_mask, 0] = target_rgb[0]
        result[color_mask, 1] = target_rgb[1]
        result[color_mask, 2] = target_rgb[2]
    ...
    logger.info(f"Function took {execution_time:.4f} seconds to execute")
    return result_img
...
def rpix(img):
    ...
    # Create color mapping dictionary for faster lookup
    color_mapping = {}
    for old_color, new_color in zip(colors_real, colors_map):
        color_mapping[hex_to_rgb(old_color)] = hex_to_rgb(new_color)
    ...
    for y in range(height):
        for x in range(width):
            ...
            if pixel_color in color_mapping:
                pixels[x, y] = color_mapping[pixel_color]
    end_time = time.time()
    execution_time = end_time - start_time
    logger.info(f"Function took {execution_time:.4f} seconds to execute")
    return img
...
def rpil(img):
    """
    Replace colors in an in-memory PIL Image object according to the mapping

    Args:
        img: PIL Image object in RGB mode

    Returns:
        PIL Image with replaced colors
    """
    start_time = time.time()
    ...
    # Create color mapping dictionary using ImageColor
    color_mapping = {}
    for old_color, new_color in zip(colors_real, colors_map):
        color_mapping[ImageColor.getrgb(old_color)] = ImageColor.getrgb(new_color)
...
p = Path("../album_art/album_art_c9dee5d8a58dd1a7ca269b25e489d5d3.jpg")
...
config = RoonFrameConfig().config
# Import the appropriate module
eink = importlib.import_module("libs.epd13in3E")
viewer = EinkViewer(config, eink)
